<template>
  <div>
    <div style="display: flex">
      <a-button size="large" class="editable-add-btn" @click="commodityVisible = true;clearInput()">
        新增商品
      </a-button>
      <a-input-search
          placeholder="请输入商品名"
    ...
    <a-table :loading="loading" :columns="columns" :data-source="data" :sortDirections="['ascend', 'descend']" rowKey="id">
      ...
      <span slot="action" slot-scope="text, record, index">
        <a-button @click="handleUpdate(record)" type="primary"> 编辑</a-button>
      <a-divider type="vertical"/>
         <a-button @click="handleDelete(record,index)" type="danger"> 删除</a-button>
      </span>
    </a-table>
