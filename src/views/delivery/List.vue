  ...
  <div>
    <a-table :loading="loading" :columns="columns" :data-source="data" bordered rowKey="id">
      <span slot="status" slot-scope="status">
        <a-tag v-if="status===0" color="#f50" style="padding: 8px;">等待审核</a-tag>
        <a-tag v-if="status===1" color="#87d068" style="padding: 8px;">正在运输</a-tag>
        <a-tag v-if="status===2" color="#2db7f5" style="padding: 8px;">配送完成</a-tag>
      </span>
      <template
      ...
      <template slot="operation" slot-scope="text, record, index">
        <div class="editable-row-operations">
          ...
          <a-button @click="() => save(record.id, index)" type="primary">保存</a-button>
          <a-popconfirm title="取消修改?" :cancelText="'否'" :okText="'是'" @confirm="() => cancel(record.id)">
            <a-button type="danger">取消</a-button>
          </a-popconfirm>
        </span>
          <span v-else>
          <a-button :disabled="editingKey !== ''" @click="() => edit(record.id)" type="primary">编辑</a-button>
        </span>
          <a-button @click="review(index)" type="dashed" v-if="record.status===0">审核</a-button>
          <a-button @click="review(index)" type="dashed" v-if="record.status===1">配送</a-button>
          <a-button @click="review(index)" type="default" v-if="record.status===2">查看</a-button>
        </div>
      </template>
    </a-table>
